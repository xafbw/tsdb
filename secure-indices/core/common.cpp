...
void splitIntoArithmeticShares(PRNG *prng, uint128_t *vals, int len, uint128_t *valShares[]) {
    for (int i = 0; i < len; i++) {
        ...
        //valShares[1][i] = vals[i];
        valShares[1][i] = vals[i] - valShares[0][i];
    }
...
uint128_t stringTouint128(string p){    
    if (p.length() > intsize / 8) {
    cout << "string out of length!";
    return 0;
    }
    uint128_t temp1=1;
    uint128_t result = 0;
...
string int128Toustring(uint128_t p) {              
    string result = "";
    unsigned char temp;
    for (int i = 0; i < intsize / 8; i++) {
    temp = 0;
    for (int j = 0; j < 8; j++) {
        temp+=int(((p >> i * 8 + j) & 1) * (int)pow(2, j));
    }
...
string int64Toustring(uint64_t p) {              
    string result = "";
    unsigned char temp;
...
void sortStringsByLength(vector<string> strs, int len) {
  for (int i = 0; i < len; i++) {
